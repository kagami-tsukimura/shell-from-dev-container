...
- SIGTERM: `ps aux`で pid を表示し、pid に対して`kill`コマンドを実行した際は、`SIGTERM`を送信している。

## パイプ

- パイプの左側の実行結果を右側で受け取って操作できる。
- 実体は、親プロセスが子プロセスを左右 2 つの子プロセスとして fork している。
- 左の子プロセスの標準出力を右の子プロセスは標準入力から読み取れる。
